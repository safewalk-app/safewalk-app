...
BEGIN
  ...
  LEFT JOIN public.emergency_contacts ec ON 
    ct.user_id = ec.user_id 
    AND ec.priority = 1 
...
-- ============================================================================
-- 4. HELPER: reset_daily_quotas (for testing/admin)
-- ============================================================================
-- Purpose: Reset daily SMS quotas (called at midnight or for testing)
-- Note: In production, quotas reset automatically via sms_logs date_trunc
...
BEGIN
  -- No action needed: quotas reset automatically via date_trunc('day')
  -- This function is a placeholder for future admin operations
  RETURN QUERY SELECT 0, 'Quotas reset automatically via date_trunc'::text;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute to service_role only
REVOKE EXECUTE ON FUNCTION public.reset_daily_quotas() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reset_daily_quotas() TO service_role;

-- ============================================================================
-- 5. CREATE INDEXES FOR RPC PERFORMANCE
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_emergency_contacts_priority_opted 
ON public.emergency_contacts(user_id, priority, opted_out)
