...
import { OtpErrorCode, getErrorTitle, getErrorType } from '@/lib/types/otp-errors';
    ...
    await triggerSendOtp(async () => {
      ...
      if (!validation.isValid || !validation.formatted) {
        setError({
          ...
          message: validation.message || 'Format invalide. Utilisez +33... ou 06...',
        });
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        <View className="flex-1 justify-between gap-8 py-4">
          ...
          <View className="gap-2">
            ...
            <Text className="text-base text-muted">Entrez votre numéro de téléphone</Text>
          </View>
          ...
          <View className="gap-4">
            ...
            {/* Phone Input Section */}
            <View className="gap-2">
              <Text className="text-sm font-semibold text-foreground">Numéro de téléphone</Text>

              {/* Input Field */}
              <View
                className={cn(
                  'flex-row items-center gap-2 rounded-2xl px-4 py-3 border',
                  error ? 'bg-error/5 border-error/30' : 'bg-surface border-border',
                )}
              >
            ...
            <View className="bg-primary/10 rounded-lg p-3 border border-primary/20">
              <Text className="text-xs text-foreground leading-relaxed">
                Nous enverrons un code de vérification par SMS. Les tarifs SMS standards
                s'appliquent.
              </Text>
            </View>
            ...
            {phoneInput && (
              ...
              >
                ...
                >
                  {validation.isValid ? '✓ Numéro valide' : validation.message || 'Numéro invalide'}
                </Text>
              </View>
            )}
          </View>
          ...
          <View className="gap-3">
            ...
            <TouchableOpacity
              ...
              style={{
                opacity: loading || !isComplete || !validation.isValid || isOnCooldown ? 0.6 : 1,
              }}
            >
              {loading ? (
                ...
                <Text className="text-base font-semibold text-background">Envoyer le code</Text>
              )}
            </TouchableOpacity>
